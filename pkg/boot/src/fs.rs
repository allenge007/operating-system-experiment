...
pub fn load_apps() -> AppList {
    ...
    loop {
        ...
        match info {
            Some(entry) => {
                let file = { 
                    /* DONE: get handle for app binary file */ 
                ...
                let elf = {
                    ...
                    let mut reg_file = match file.into_type().expect("Failed to into_type") {
                        FileType::Regular(f) => f,
                        _ => panic!("Invalid file type"),
                    };
                    ElfFile::new(load_file(&mut reg_file)).expect("Failed to parse ELF file")
                };
